...
public class ConsoleNetworkObserver implements PacketObserver {
	...
	public void packetReceived(byte[] data, SocketAddress saddr) {
		if (data[OffsetAndSizeConstants.PACKET_TYPE_OFFSET] == ServerPacketType.INITIALIZER) {
			...
			System.out.println(v);
			*/
		} else if (data[OffsetAndSizeConstants.PACKET_TYPE_OFFSET] == ServerPacketType.MESSAGE) {
			...
			System.out.println("Unhandled packet type: " + data[OffsetAndSizeConstants.PACKET_TYPE_OFFSET] + ", length: " + data.length);
